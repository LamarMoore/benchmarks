# * Find CxxTest Find the CxxTest suite and declare a helper macro for creating unit tests and integrating them with
...
# OUTPUT Variables
#
# CXXTEST_FOUND True if the CxxTest framework was found CXXTEST_INCLUDE_DIR Where to find the CxxTest include directory
# CXXTEST_PERL_TESTGEN_EXECUTABLE The perl-based test generator. CXXTEST_PYTHON_TESTGEN_EXECUTABLE The python-based test
# generator.
#
# MACROS for optional use by CMake users:
#
# CXXTEST_ADD_TEST(<test_name> <gen_source_file> <input_files_to_testgen...>) Creates a CxxTest runner and adds it to
# the CTest testing suite Parameters: test_name               The name of the test input_files_to_testgen  The list of
# header files containing the CxxTest::TestSuite's to be included in this runner
#
# The variable TESTHELPER_SRCS can be used to pass in extra (non-test) source files that should be included in the test
# executable.
#
# The variable CXXTEST_EXTRA_HEADER_INCLUDE can be used to pass an additional header to the --include optional of the
# cxxtestgen command.
#
# The variable CXXTEST_OUTPUT_DIR can be used to specify the directory for the generated files. The default is
# CMAKE_CURRENT_BINARY_DIR.
#
# The variable CXXTEST_ADD_PERFORMANCE can be used to add the Performance tests to ctest. This also passes
# --xunit-printer argument to the cxxtestgen step
#
# #==============
# Example Usage:
#
...
#
# set(TESTHELPER_SRCS HelperClass1.cc HelperClass2.cc) CXXTEST_ADD_TEST(unittest_foo foo_test.cc
# ${CMAKE_CURRENT_SOURCE_DIR}/foo_test.h) target_link_libraries(unittest_foo foo) # as needed endif()
#
# This will (if CxxTest is found): 1. Invoke the testgen executable to autogenerate foo_test.cc in the binary tree from
# "foo_test.h" in the current source directory. 2. Create an executable and test called unittest_foo. 3. Files specified
# in TESTHELPER_SRCS will also be compiled and linked in.
#
# #=============
# Example foo_test.h:
#
...
#
# class MyTestSuite : public CxxTest::TestSuite { public: void testAddition( void ) { TS_ASSERT( 1 + 1 > 1 );
# TS_ASSERT_EQUALS( 1 + 1, 2 ); } };
#

# =============================================================================
# Copyright 2008-2009 Kitware, Inc. Copyright 2008-2009 Philip Lowman <philip@yhbt.com>
#
...
# Version 1.2 (3/2/08) Included patch from Tyler Roscoe to have the perl & python binaries detected based on
# CXXTEST_INCLUDE_DIR Version 1.1 (2/9/08) Clarified example to illustrate need to call target_link_libraries() Changed
...
macro(CXXTEST_ADD_TEST _cxxtest_testname)
  ...
  add_custom_command(
    ...
    COMMAND ${Python_EXECUTABLE} ${CXXTEST_TESTGEN_EXECUTABLE} --root ${_printer} --world ${_cxxtest_testname}
            ${_cxxtest_include} -o ${_cxxtest_real_outfname}
  )
  ...
  foreach(part ${ARGN})
    ...
    add_custom_command(
      ...
      COMMAND ${Python_EXECUTABLE} ${CXXTEST_TESTGEN_EXECUTABLE} --part --world ${_cxxtest_testname} -o ${_cxxtest_cpp}
              ${_cxxtest_h}
    )
...
find_path(
  ...
  PATHS ${PROJECT_SOURCE_DIR}/tools/cxxtest ${PROJECT_SOURCE_DIR}/../tools/cxxtest
...
file(GLOB_RECURSE CXXTEST_PYTHON_FILES ${PROJECT_SOURCE_DIR}/tools/cxxtest/python/*.py)
